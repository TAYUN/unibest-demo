...
// 图片格式化函数
// 获取套餐列表的接口
import { getMealListAPI } from '@/service/user/meal'
...
const getData = async (): Promise<any[]> => {
  const res = await getMealListAPI({
  ...
  const dataList = res.data.items.map((item: any) => ({
    ...
    isNearBy: item.is_near_by,
    merchant: {
      ...item.community_merchant,
      avatar: formatImageUrl(item.community_merchant.avatar),
    },
  }))
  const totalPage = res.data.pageInfo.totalPage
  // 更新分页信息
...
const isLoadComplete = computed(() => imageLoadCount.value === totalImages.value) // 判断是否所有图片加载完成
